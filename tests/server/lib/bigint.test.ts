...
describe('formatMinor', () => {
  it('formats 1 USDC correctly', () => {
    const result = formatMinor('10000000');
    expect(result).toContain('1');
  });

  it('formats with symbol', () => {
    const result = formatMinor('50000000', { symbol: '$' });
...
describe('humanToMinor', () => {
  it('converts whole number', () => {
    expect(humanToMinor('5')).toBe('50000000');
    expect(humanToMinor('1')).toBe('10000000');
    expect(humanToMinor('100')).toBe('1000000000');
  });

  it('converts decimal', () => {
    expect(humanToMinor('5.5')).toBe('55000000');
    expect(humanToMinor('0.50')).toBe('5000000');
    expect(humanToMinor('1.123456')).toBe('11234560');
  });
});

describe('minorToHuman', () => {
  it('converts minor to human', () => {
    expect(minorToHuman('50000000')).toBe('5.0000000');
    expect(minorToHuman('10000000')).toBe('1.0000000');
    expect(minorToHuman('5000000')).toBe('0.5000000');
  });
});
