...
const SALT = crypto.createHash('sha256').update('016-padala-payment-registry-v1').digest();
...
function outputPath(stage, network) {
  return path.resolve(ROOT, `contracts/payment-registry/${network}-${stage}-assembled.xdr`);
}
